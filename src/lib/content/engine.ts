...
export function analyzeSeo(
  ...
  // ----- 1. 제목 키워드 (10점) -----
  {
    ...
    if (titleHasKeyword) {
      ...
      if (keywordPos >= 0 && keywordPos <= 10) {
        score += 5
        strengths.push('제목 앞쪽에 키워드가 잘 배치되어 있습니다')
      } else {
        score += 2
        improvements.push('키워드를 제목 앞쪽(10자 이내)에 배치하면 더 좋습니다')
      }
    } else {
      improvements.push('제목에 핵심 키워드를 포함하세요')
    }
    categories.push({ name: '제목 키워드', score, maxScore: 10, details: titleHasKeyword ? `키워드 위치: ${keywordPos + 1}번째 글자` : '키워드 미포함' })
  }
  ...
  // ----- 2. 제목 길이 (10점) -----
  {
    ...
    if (titleLen >= 20 && titleLen <= 40) {
      score = 10
      strengths.push(`제목 길이 최적 (${titleLen}자)`)
    } else if (titleLen >= 15 && titleLen <= 50) {
      score = 7
    } else if (titleLen >= 10) {
      score = 4
      improvements.push(`제목이 ${titleLen > 50 ? '너무 깁니다' : '너무 짧습니다'} (권장: 20~40자)`)
    } else {
      score = 1
      improvements.push('제목이 너무 짧습니다 (최소 15자 권장)')
    }
    categories.push({ name: '제목 길이', score, maxScore: 10, details: `${titleLen}자 (권장: 20~40자)` })
  }
  ...
  // ----- 3. 소제목 구조 (10점) -----
  {
    ...
    if (h2Count >= 3 && h3Count >= 2) {
      score = 10
      strengths.push(`H2(${h2Count}개) + H3(${h3Count}개) 구조 우수`)
    } else if (h2Count >= 3) {
      score = 8
    } else if (h2Count >= 2) {
      score = 5
      ...
      score = 3
      improvements.push('소제목(H2)을 3개 이상 사용하세요')
    } else {
      score = 0
      improvements.push('소제목(##)을 사용하여 콘텐츠를 구조화하세요')
    }
    categories.push({ name: '소제목 구조', score, maxScore: 10, details: `H2: ${h2Count}개, H3: ${h3Count}개 (총 ${totalHeadings}개)` })
  }
  ...
  // ----- 4. 키워드 밀도 (10점) -----
  {
    ...
    if (density >= 0.5 && density <= 2.5) {
      score = 10
      strengths.push(`키워드 밀도 최적 (${density.toFixed(1)}%)`)
    } else if (density > 0 && density < 0.5) {
      score = 5
      improvements.push('키워드 사용 빈도를 조금 더 늘려보세요')
    } else if (density > 2.5 && density <= 4) {
      score = 6
      improvements.push('키워드가 약간 과도합니다. 동의어로 대체하세요')
    } else if (density > 4) {
      score = 2
      improvements.push('키워드 스터핑 위험! 자연스러운 표현으로 교체하세요')
    } else {
      score = 0
      improvements.push('본문에 핵심 키워드를 포함하세요')
    }
    categories.push({ name: '키워드 밀도', score, maxScore: 10, details: `${keywordCount}회 (밀도: ${density.toFixed(1)}%)` })
  }
  ...
  // ----- 5. 키워드 분포 (10점) -----
  {
    ...
    if (coverage === 3) {
      score = 10
      strengths.push('키워드가 본문 전체에 고르게 분포되어 있습니다')
    } else if (coverage === 2) {
      score = 6
      if (!inFirst) improvements.push('도입부에 키워드를 포함하세요')
      if (!inLast) improvements.push('마무리 부분에 키워드를 포함하세요')
    } else if (coverage === 1) {
      score = 3
      improvements.push('키워드를 본문 전반에 걸쳐 배치하세요')
    } else {
      score = 0
    }
    categories.push({ name: '키워드 분포', score, maxScore: 10, details: `앞(${inFirst ? 'O' : 'X'}) / 중(${inMiddle ? 'O' : 'X'}) / 뒤(${inLast ? 'O' : 'X'})` })
  }
  ...
  // ----- 6. 콘텐츠 길이 (10점) -----
  {
    ...
    if (contentLength >= 2500) {
      score = 10
      strengths.push(`콘텐츠 분량 우수 (${contentLength.toLocaleString()}자)`)
    } else if (contentLength >= 1800) {
      score = 8
    } else if (contentLength >= 1200) {
      score = 5
      improvements.push('콘텐츠 분량을 2,000자 이상으로 늘리세요')
    } else if (contentLength >= 800) {
      score = 3
      improvements.push('콘텐츠가 짧습니다 (최소 1,500자 권장)')
    } else {
      score = 1
      improvements.push('콘텐츠가 너무 짧습니다 (2,000자 이상 권장)')
    }
    categories.push({ name: '콘텐츠 길이', score, maxScore: 10, details: `${contentLength.toLocaleString()}자 (권장: 2,000~3,000자)` })
  }
  ...
  // ----- 7. 멀티미디어 (10점) -----
  {
    ...
    if (imageCount >= 4) {
      score = 10
      strengths.push(`이미지 ${imageCount}개 배치 (우수)`)
    } else if (imageCount >= 2) {
      score = 7
    } else if (imageCount >= 1) {
      score = 4
      improvements.push('이미지를 3개 이상 포함하면 체류 시간이 증가합니다')
    } else {
      score = 0
      improvements.push('이미지를 추가하여 콘텐츠의 시각적 매력을 높이세요')
    }
    categories.push({ name: '멀티미디어', score, maxScore: 10, details: `이미지 ${imageCount}개` })
  }
  ...
  // ----- 8. 가독성 요소 (10점) -----
  {
    ...
    if (hasBold) score += 3
    if (hasList || hasNumberedList) score += 3
    if (paragraphs >= 8) score += 4
    else if (paragraphs >= 5) score += 2

    if (score >= 8) strengths.push('가독성 요소 (볼드, 리스트, 문단 분리) 우수')
    if (!hasBold) improvements.push('**볼드** 처리로 핵심 내용을 강조하세요')
    if (!hasList && !hasNumberedList) improvements.push('리스트를 활용하면 가독성이 향상됩니다')

    categories.push({ name: '가독성 요소', score, maxScore: 10, details: `볼드(${hasBold ? 'O' : 'X'}), 리스트(${hasList || hasNumberedList ? 'O' : 'X'}), 문단(${paragraphs}개)` })
  }
  ...
  // ----- 9. 관련 키워드 활용 (10점) -----
  {
    ...
    if (additionalKeywords && additionalKeywords.length > 0) {
      ...
      if (usageRate >= 0.7) {
        score = 10
        strengths.push(`관련 키워드 ${used.length}/${additionalKeywords.length}개 활용 (우수)`)
      } else if (usageRate >= 0.4) {
        score = 6
      } else if (usageRate > 0) {
        score = 3
        improvements.push('관련 키워드를 더 많이 활용하세요')
      } else {
        score = 0
        improvements.push('관련 키워드를 본문에 포함하여 주제 전문성을 높이세요')
      }
      categories.push({ name: '관련 키워드', score, maxScore: 10, details: `${used.length}/${additionalKeywords.length}개 활용` })
    } else {
      // 관련 키워드 없으면 만점 처리 (감점 없음)
      score = 10
      categories.push({ name: '관련 키워드', score, maxScore: 10, details: '관련 키워드 미지정 (기본 만점)' })
    }
  }
  ...
  // ----- 10. 태그 & CTA (10점) -----
  {
    ...
    if (hasHashtags) {
      ...
      if (tagCount >= 5) score += 6
      else if (tagCount >= 3) score += 4
      else score += 2
    } else {
      improvements.push('관련 태그(#)를 5~10개 추가하세요')
    }

    if (hasCta) {
      score += 4
      strengths.push('독자 참여 유도 문구 포함')
    } else {
      score += 0
      improvements.push('마무리에 댓글/공감 유도 문구를 추가하세요')
    }

    categories.push({ name: '태그 & CTA', score, maxScore: 10, details: `태그(${hasHashtags ? 'O' : 'X'}), CTA(${hasCta ? 'O' : 'X'})` })
  }
...
export function generateAutoTags(keyword: string, content: string, maxTags: number = 10): string[] {
  ...
  const stopwords = new Set([
    ...
    '것입니다', '블로그', '포스팅', '오늘은', '안녕하세요', '여러분', '이번에',
    '대해서', '통해서', '위해서', '함께', '시작', '다양한', '중요한', '필요한',
    '어떤', '그런', '이런', '저런', '모든', '각각', '또한', '매우', '정말',
    '아주', '너무', '조금', '약간', '먼저', '다음', '마지막', '관련', '경우',
  ])
