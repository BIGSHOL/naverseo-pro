...
### 🔴 SEO 점수 채점 기준 (100점 만점, 13개 항목)
이 글은 아래 13개 항목으로 자동 채점됩니다. 각 항목의 배점을 숙지하고 **모든 항목에서 만점**을 목표로 작성하세요:

| # | 항목 | 배점 | 핵심 기준 |
|---|------|------|----------|
| 1 | 제목 키워드 | 8점 | 키워드가 제목 앞쪽(15자 이내)에 위치 |
| 2 | 제목 길이 | 7점 | 20~40자 (최적 25~35자) |
| 3 | 소제목 구조 | 8점 | H2 3~5개 + H3 2개 이상 계층화 |
| 4 | 키워드 밀도 | 8점 | 본문 내 키워드 0.5~2% (2000자 기준 5~8회) |
| 5 | 키워드 분포 | 7점 | 전반부·중반부·후반부 각 구간에 키워드 포함 |
| 6 | 콘텐츠 길이 | 7점 | 1,500자 이상 (최적 2,000~3,000자) |
| 7 | 멀티미디어 | 7점 | [이미지: 설명] 3개 이상 (최적 5~7개) |
...
## 🔴 중복 표현 절대 금지 (최우선 규칙)
- 여러 patch에서 같은 문구·같은 표현·같은 문장 구조를 반복하지 마세요
- 각 patch의 "replace"는 서로 다른 표현과 어휘를 사용해야 합니다
- 특히 다음 패턴을 절대 반복하지 마세요:
  × "~해 보세요", "~를 확인해 보세요" 등 동일한 권유 문구를 여러 곳에 사용
  × 같은 접속사·부사를 연속 patch에서 반복 (예: "특히", "무엇보다", "또한")
  × 키워드를 포함한 동일 문장 패턴을 여러 번 삽입
  × CTA(공감/댓글 유도) 문구를 본문 곳곳에 중복 배치
- patch 작성 전 이미 작성한 patch들을 검토하고, 표현이 겹치면 다른 말로 바꾸세요
- "append"에 넣은 내용이 본문 patch와 중복되지 않도록 주의하세요
