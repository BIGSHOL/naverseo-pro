/**
 * 네이버 알고리즘 추정 점수 계산기 (v14)
 *
 * 4축(40:25:20:15) + 검색 노출력(참고)을 가중 합산하여
...
// 핵심: 콘텐츠 품질 + 주제 전문성 + 사용자 반응(체류시간, 댓글, 공감)
const DIA_WEIGHTS: Record<string, number> = {
  '주제 전문성': 0.25,   // C-Rank 연계 — 주제 부합도
  '콘텐츠 품질': 0.30,   // 글 품질·이미지·구조 → 의도 부합도 핵심
  '활동 신뢰도': 0.20,   // 꾸준한 활동 → 신선한 콘텐츠
  '사용자 반응': 0.20,   // 체류시간·댓글·공감 → 만족도 지표
  '검색 노출력': 0.05,   // 참고 지표 — 키워드 부합도 약간 반영
}
...
const CRANK_WEIGHTS: Record<string, number> = {
  ...
  '활동 신뢰도': 0.30,   // 운영 기간·꾸준한 포스팅·규칙성
  '콘텐츠 품질': 0.15,   // 콘텐츠 깊이·경험 정보·구조
  '사용자 반응': 0.15,   // 댓글·공감 = 블로그 인지도
  '검색 노출력': 0.05,   // 참고 지표 — 검증된 권위 약간 반영
}
...
function calculateScore(
  ...
  // v14: 4축 + 참고 지표 순서
  const CATEGORY_ORDER = ['주제 전문성', '활동 신뢰도', '사용자 반응', '콘텐츠 품질', '검색 노출력']
