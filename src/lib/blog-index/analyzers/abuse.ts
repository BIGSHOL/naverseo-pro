/**
 * 블로그 지수 - P. 어뷰징 감점 분석 — 최대 -20점 (v8: 스팸 키워드 + 외부 링크 + 본문 중복 강화)
 *
 * 5가지 어뷰징 유형 감지:
...
export function analyzeAbuse(
  ...
  if (scrapedData && scrapedData.size >= 3) {
    ...
    if (totalComparisons > 0) {
      ...
      if (simRate >= 0.4) {
        score -= 6
        details.push(`본문 콘텐츠 중복 심각: ${Math.round(simRate * 100)}%의 글 쌍이 유사 (자기 표절 의심)`)
        flags.push('content_duplication')
        contentDupDetected = true
      } else if (simRate >= 0.2) {
        score -= 3
        details.push(`본문 콘텐츠 일부 중복: ${Math.round(simRate * 100)}%의 글 쌍이 유사`)
        flags.push('content_duplication')
        contentDupDetected = true
      }
    }
  ...
  if (detectedSpam.length >= 3) {
    score -= 5
    details.push(`스팸 키워드 다수 감지: ${detectedSpam.slice(0, 5).join(', ')} (${detectedSpam.length}종)`)
    flags.push('spam_keywords')
  } else if (detectedSpam.length >= 1) {
    score -= 2
    details.push(`스팸성 키워드 주의: ${detectedSpam.join(', ')}`)
    flags.push('spam_keywords')
  }
  ...
  if (scrapedData && scrapedData.size > 0) {
    ...
    if (postsWithMeta.length > 0) {
      ...
      if (suspiciousCount >= 5) {
        score -= 4
        details.push(`광고성/단축 URL ${suspiciousCount}개 감지 - 신뢰도에 부정적 영향`)
        flags.push('suspicious_links')
      } else if (suspiciousCount >= 2) {
        score -= 2
        details.push(`단축 URL ${suspiciousCount}개 감지 주의`)
        flags.push('suspicious_links')
      }

      if (avgExternal >= 5) {
        score -= 3
        details.push(`외부 링크 과다 (포스트당 평균 ${avgExternal.toFixed(1)}개) - 스팸 신호`)
        if (!flags.includes('suspicious_links')) flags.push('excessive_external_links')
      } else if (avgExternal >= 3) {
        score -= 1
        details.push(`외부 링크 다소 많음 (포스트당 평균 ${avgExternal.toFixed(1)}개)`)
      }
    }
  }
