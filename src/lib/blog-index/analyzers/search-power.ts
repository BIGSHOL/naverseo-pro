/**
 * 블로그 지수 - 검색 노출력 (참고 지표, 점수 미반영)
 *
 * v14: 검색 노출은 C-Rank+DIA의 부산물(결과)이므로 점수에서 제외.
 *      순환 논리 제거 → 4축(전문성/활동/반응/품질)만으로 블로그 역량 측정.
 *      검색 데이터는 노출 검증(ExposureVerification)에서 참고 지표로만 활용.
 *
 * 분석 항목: 검색 순위 + 노출률 + TOP10 + 제목 최적화 + 감점 패턴
 * 점수 반영: 없음 (totalScore 합산 제외)
 */
