/**
 * NaverSEO Pro - 블로그 지수 측정 엔진 v17
 *
 * v17 점수 체계: 4축 C-Rank 업계 기준 배분 = 100점
 *
 * 4대 분석 축 (40:20:25:15):
 * 1. 주제 전문성 - 40점: 일관성(11), 카테고리(7), 시리즈(8), 전문용어(7), 품질일관성(7), 유사도(-5), 중복(-5)
 * 2. 활동 신뢰도 - 20점: 규칙성(5), 빈도(4), 최근성(2), 누적(4), 운영기간(5), 스팸(-3), 외부링크(-3), 일괄발행(-3), 과다발행(-2)
 * 3. 사용자 반응 - 25점: 방문자(5), 댓글(8), 공감(5), 이웃(5), 체류(2), 벽텍스트(-1), 광고성(-1)
 ...
 *
 * v16→v17 변경:
 * - 활동 신뢰도 25→20점: 매일 포스팅하는 저품질 블로그가 과대평가되는 문제 해결
 * - 사용자 반응 20→25점: 방문자 수(5점) 신규 추가, 실제 인기도 반영 강화
 * - 방문자 점수: NVisitorgp4Ajax 30일 평균 기반 (500+→5, 200+→4, 80+→3, 30+→2, 10+→1)
 */
