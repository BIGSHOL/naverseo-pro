...
const valuePoints = [
  ...
  { icon: BarChart3, text: '13개 항목 SEO 정밀 분석' },
    ...
    <section className="py-20 bg-primary/5">
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center">
        ...
        <p className="mt-4 text-base text-muted-foreground sm:text-lg">
          ...
          매달 <span className="font-semibold text-foreground">50시간 이상</span>을 절약하고,{' '}
          <br className="sm:hidden" />
          상위 노출 전략에 집중할 수 있습니다.
        </p>

        {/* 가치 요약 */}
        <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {valuePoints.map((point) => (
            <div key={point.text} className="flex flex-col items-center gap-2 rounded-lg bg-background p-3 sm:p-4 shadow-sm">
              <point.icon className="h-5 w-5 text-primary" />
              <span className="text-center text-[11px] sm:text-xs font-medium leading-tight">{point.text}</span>
            </div>
          ))}
        </div>
