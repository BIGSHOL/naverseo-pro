...
export function InstagramTabCaption({ userPlan }: Props) {
  ...
  // 페이지 복귀 시 캐시 복원
  useEffect(() => {
    const cached = loadPageCache<{ result: CaptionResult; content: string }>('instagram-caption')
    if (cached) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch('/api/ai/instagram', {
        ...
        body: JSON.stringify({ mode: 'caption', content: content.trim() }),
      })
      ...
      setResult(data)
      savePageCache('instagram-caption', { result: data, content: content.trim() })
