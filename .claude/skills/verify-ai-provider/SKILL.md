...
| `src/app/api/ai/instagram/route.ts` | 인스타그램 변환 (Gemini 고정 — callGemini 직접 사용) |
| `src/lib/ai/insights.ts` | AI 인사이트 (Gemini 고정 — callGemini 직접 사용, 4개 함수) |
...
**파일:** Gemini만 사용하는 4개 파일

**검사:** 키워드/발굴/인스타그램/인사이트 라우트에서 `callGemini`을 직접 사용하고, `callAI`/`getUserAiProvider`를 사용하지 않는지 확인합니다.
...
Grep pattern="callGemini" path="src/app/api/ai/instagram" glob="route.ts" output_mode="content"
Grep pattern="callGemini" path="src/lib/ai/insights.ts" output_mode="content"
...
Grep pattern="callAI|getUserAiProvider" path="src/app/api/ai/instagram" glob="route.ts" output_mode="content"
Grep pattern="callAI|getUserAiProvider" path="src/lib/ai/insights.ts" output_mode="content"
```

**PASS:** keywords, opportunities, instagram, insights 라우트가 `callGemini` 직접 사용, `callAI`/`getUserAiProvider` 미사용
